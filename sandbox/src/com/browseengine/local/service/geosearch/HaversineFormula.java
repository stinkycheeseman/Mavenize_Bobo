...
public class HaversineFormula {
	 ...
	 * computes the Haversine distance between 2 points, in miles.
	 *
	 * <p>
	 * source: 
	 ...
	 *</div>
	 */
	public static float computeHaversineDistanceMiles(double lon1, double lat1, 
	 ...
	 * might be screwed up near the poles.
	 * @param radius_d to the caller, this is the radius of the search, in miles.  
	 *     however, we treat this as our value "d" in the Haversine formula.
	 *     hence, the name reflects both.
	 * @return the latitudinal displacement, in radians
	 */
	public static double computeLatBoundary(double lon1, double lat1, 
					    float radius_d) {
		//	 d = R*(2*sin^-1(min(1,sqrt(sin^2(dlat/2)+cos(lat1)*cos(lat2)*sin^2(dlon/2)))))
	...
	/**
	 * all inputs and outputs are in radians.
	 * might be screwed up near the poles.  lat1 can't be PI/2, for example.
	 */
	public static double computeLonBoundary(double lon1, double lat1,
					    float radius_d) {
        //	 d = R*(2*sin^-1(min(1,sqrt(sin^2(dlat/2)+cos(lat1)*cos(lat2)*sin^2(dlon/2)))))
        ...
        //	 dlon is now on -PI/2,PI/2 (not sure of fenceposts, documentation isn't specific)
		if (dlon < 0) {
			dlon += Math.PI;
		}
		return dlon;
